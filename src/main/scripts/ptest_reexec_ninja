...
# method#2
# cat ../_dx_stats1 |cut -d '/' -f 5-|xargs -n1 -IX basename X .q.out|sort|uniq|sed -r 's/(.*)/Test*CliDriver#*[\1]/'|paste -s -d,

#method #3

# |grep testCliDriver|sed 's/org.apache.hadoop.hive.cli.//'|cut -d ' ' -f1|tr '.' '#'|
